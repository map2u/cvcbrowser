...
ol3.map2u.createMap = function (mapId) {
    ...
    app.BottomFeatureListControl = function (opt_options) {
        var options = opt_options || {};
        var element = document.createElement('div');
        element.className = 'ol-bottom-feature-list ol-unselectable ol-control hidden';
        ol.control.Control.call(this, {
            element: element,
            target: options.target
        });
    ...
    map = new ol.Map({
        controls: ol.control.defaults({
            ...
            new ol.control.ScaleLine(), mousePositionControl, new app.RotateNorthControl(), new app.BottomFeatureListControl()//, new app.MeasurementControl()
        ]),
            ...
            if (typeof fl.feature.get("features") !== 'undefined') {
                ...
                if (features.length === 1 && fl.layer.get('type') === 'story') {
                    ...
                    $.when(app.map2u.activateMapSidebar("feature_information", 'Story')).then(function () {
                        app.map2u.setMapSidebarContent("feature_information", 'homepage_leftsidebar_view', {_locale: app.locale, id: id, view: "story"});
                    });
                } else {
...
//                            var img = style.getImage();
                            ...
                            var item = $("<div class='feature-list-item' data-id='" + id + "'><img alt=" + name + " src='" + img + "'  height='36' ></div>");
                ...
                if (fl.feature.get("type") === 'story') {
                    ...
                    if (typeof id === 'undefined' || id === null || id.trim() === '') {
                        $.when(app.map2u.activateMapSidebar("feature_information", 'Create a new story')).then(function () {
                            app.map2u.setMapSidebarContent("feature_information", 'draw_createstory', {_locale: app.locale});
                        });
                    } else {
                        $.when(app.map2u.activateMapSidebar("feature_information", 'Story')).then(function () {
                            app.map2u.setMapSidebarContent("feature_information", 'homepage_leftsidebar_view', {_locale: app.locale, id: id, view: "story"});
    ...
    $(window).on('resize', function () {
        ...
        $('div#' + mapId).height($(window).height() - 145);
        $('div#page-right-sidebar').height($(window).height() - 112);
        $('div#ol3map-sidebar').height($(window).height() - 112);
        ol3.map2u.map.updateSize();
        ol3.map2u.updateBottomFeatureListPosition();

        //       $('div#' + mapId + " .ol-bottom-feature-list").css("left", ($('div#' + mapId + " .ol-scale-line .ol-scale-line-inner").width() + 25) + "px");
        //       $('div#' + mapId + " .ol-bottom-feature-list").width($('div#' + mapId).width() - ($('div#' + mapId + " .ol-scale-line .ol-scale-line-inner").width() + 25) - 50);
...
app.map2u.setMapSidebarContent = function (id, url, parameters) {
    ...
    $.ajax({
        url: Routing.generate(url, parameters),
        type: 'GET',
        complete: function () {
    ...
    $("li#viewingscale ul li a").click(function () {
        ...
        if (parseInt(scale) === 0) {
            ...
            $.when(app.map2u.activateMapSidebar("feature_information", 'Map bookmark')).then(function () {
                ...
                app.map2u.setMapSidebarContent("feature_information", 'useraccount_mapbookmark', {_locale: app.locale});
            });
            return;
...
function createSearchFeatureIcon(map, lng, lat, address) {
