 ...
 */
Ext.define('GeoExt.controller.Map', {
    extend: 'Ext.app.Controller',

    models: ['Watershed','Subwatershed','Station'],
    stores: ['Watersheds','Subwatersheds','Stations'],

    refs: [
        {ref: 'watershedGrid', selector: 'watershedgrid'},
        {ref: 'subwatershedGrid', selector: 'subwatershedgrid'},
        {ref: 'stationGrid', selector: 'stationgrid'}
    ],

    init: function() {
        var me = this;

        me.getWatershedsStore().on({
            scope: me,
            load : me.onWatershedsStoreLoad
        });
        me.getSubwatershedsStore().on({
            scope: me,
            load : me.onSubwatershedsStoreLoad
        });
        me.getStationsStore().on({
            scope: me,
            load : me.onStationsStoreLoad
        });
    ...
    onMapPanelBeforeRender: function(mapPanel, options) {
        ...
        mapPanel.map.addControls([new OpenLayers.Control.DragFeature(vecLayer, {
            ...
            onComplete: function(feature, px) {
                var store = me.getWateershedsStore();
