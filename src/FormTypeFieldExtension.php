...
//echo $baseType."<br>";
     ...
     //       var_dump($sonataAdmin);
            $attr = $view->vars['attr'];

            if (!isset($attr['class']) && isset($sonataAdmin['class'])) {
                $attr['class'] = $sonataAdmin['class'];
            }
            if($baseType=='file' && ($attr['multiple']==true || strtolower($attr['multiple'])=='multiple' ))
            {
            //  var_dump($block_prefixes)."<br>";
              $sonataAdmin['full_name']=$sonataAdmin['name'].'[]';
            //   var_dump($sonataAdmin)."<br>";
           //   var_dump($attr)."<br>";
            }
            $view->vars['attr'] = $attr;
        } else {
