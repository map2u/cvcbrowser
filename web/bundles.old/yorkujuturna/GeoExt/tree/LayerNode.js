...
Ext.define('GeoExt.tree.LayerNode', {
  ...
  //      if (target.get('radioGroup') && !layer.isBaseLayer) {
  //      alert(target.get('radioGroup'));

  //          target.set('radioGroup', 'gx_layer');
  //      }
  ...
  //              alert(checked);
//                alert(node.get('layer').getVisibility());
        ...
        if(checked && layer.name=='Subwatersheds')
        {
		for(var i=0;i<layer.map.layers.length;i++)
		{
                     if(layer.map.layers[i].name=='Watersheds')
			layer.map.layers[i].setVisibility(false);
		}
        }
        if(checked && layer.name=='Watersheds')
        {
		for(var i=0;i<layer.map.layers.length;i++)
		{
                     if(layer.map.layers[i].name=='Subwatersheds')
			layer.map.layers[i].setVisibility(false);
		}
        }
